<div class='comment-form'>
    <form method=post > 
        <input type=hidden name="redirect" value="<?=$pagekey == 'comments' ? $this->url->create('comment') : $this->url->create('hem')?>">
        <input type=hidden name="pagekey" value="<?=$pagekey?>">
        <hr>
        <p><label><br/><textarea name='content' placeholder="Comment:"><?=$content?></textarea></label></p>
       ...
       <input type='text' name='web' placeholder="Website:" value='<?=$web?>'/>
        ...
        <p class=buttons>
            <input type='submit' class="button"  name='doCreate' value='Comment'  onClick="this.form.action = '<?=$this->url->create('comment/add')?>'"/>
